...
export interface TokenStorage {
    ...
    updateTradePerformanceOnSell(tokenAddress: string, recommenderId: string, buyTimeStamp: string, sellDetails: any, isSimulation: boolean): boolean;
    ...
    saveTradeData(data: TradeData): Promise<boolean>;
    closeConnection(): void;
}

export class TrustScoreDatabase implements TokenStorage {
    private db: Database;
    ...
    constructor(db: Database) {
        ...
        const tables = this.db
            .prepare(
                "SELECT name FROM sqlite_master WHERE type='table' AND name IN ('recommenders', 'recommender_metrics', 'token_performance', 'token_recommendations', 'recommender_metrics_history', 'trade_data');"
            )
            .all();
        if (tables.length !== 6) {
  ...
  `;
    ...
    async saveTradeData(data: TradeData): Promise<boolean> {
        try {
            ...
            stmt.run(
                ...
                data.riskMetrics.liquidityRisk,
                data.riskMetrics.holderConcentration,
                data.riskMetrics.priceImpact,
